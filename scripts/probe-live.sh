...
mcp() {
  curl -s --max-time 45 -X POST "$BASE/mcp" \
...
i = raw.find('{')
try:
    data = json.loads(json.loads(raw[i:])['result']['content'][0]['text'])
except Exception as e:
    print(f'FAIL (unparseable: {e})'); sys.exit()
pubs = data if isinstance(data, list) else [data]
try:
    print('PASS' if ($expr) else 'FAIL')
except Exception as e:
    print(f'FAIL (probe error: {e})')
")
  printf '%-52s %s\n' "$name" "$verdict"
...
check "1 fusion: both publishers get slots" search \
  ...
  "len(pubs) >= 2 and all(len(p.get('hits', [])) >= 5 for p in pubs)"
...
check "7 coverage: two signed publishers" coverage '{}' \
  "len(pubs) >= 2 and all(p.get('works', 0) > 0 and p.get('envelope', {}).get('freshness', {}).get('stamp_signature_valid') is True for p in pubs)"
