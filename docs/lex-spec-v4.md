...
| **D75** | Search enrichment is a build input, never legal evidence or a runtime database. A disposable LLM workbench reconciles the signed-index inventory to corpus/publisher records and derives work-level aliases, descriptions, concepts, synonyms and candidate practice areas with evidence and provenance. Publisher-confirmed or reviewed aliases may receive exact-name rank; only validated model-derived discovery fields enter as weak recall signals and cannot become public filter truth without separate approval. The rebuilt signed index stores compact work-level FTS and, if proven useful, one work discovery vector, plus a separate enrichment digest. Publisher text, official titles/identifiers, dates, hierarchy, status, relationships, occurrences and hashes are unwritable through this path; pre/post hash, read and diff invariants gate publication. | building; experiment in `docs/retrieval-agent-enrichment-plan.md` |
| **D76** | Exact navigation remains deterministic. A named work is resolved before generation and pure navigation may bypass the model. Descriptive questions use one Microsoft Agent Framework legal-research agent over the same transport-neutral `McpCore`, with typed clarification, no more than two search attempts, exact `as_of`/article validation, bounded restorable memory, deterministic citation validation and a judge only for synthesized prose. Agent Framework is the accepted implementation boundary; the experiment selects its orchestration. The current D45 assistant remains live until repeated correctness, temporal, latency, token and cost gates pass; a multi-agent hierarchy and chatbot-first navigation remain rejected. | building; supersedes D45 only after evidence |
| **D77** | Search results use jurisdiction as the only top-level disjoint partition. Luxembourg and EU remain visible in all-scope results; matching passages nest under their work. Practice area, hierarchy, source class, legal form, legal status and language stay as jurisdiction-scoped facets because they overlap or cut across one another. Empty or single-choice facets are hidden after scope selection, incompatible state is cleared, and URLs preserve the chosen scope. | planned; UX experiment required |
