...
| **D46** | Derived consumption layer `lex-articles` (schema `lex-articles/1`): per-provision Markdown+JSON deterministically extracted from the evidence repos into a separate HEAD-is-the-contract repo. Profiles are versioned, IMMUTABLE, permanently runnable (`akn-lu/1`, `akn-lu/2`, `akn-lu-identical-scl-duplicate/1`, `xhtml-eu/1`; changes = a new profile beside the old, enforced by frozen fingerprints). `akn-lu/2` preserves official publisher-structural empty articles as signed, non-searchable coverage evidence while retaining ordinary articles as provisions; the bounded duplicate profile handles only the one measured byte-identical `scl:*` presentation defect and leaves publisher evidence unchanged. Publisher-minted anchors only; spans in Unicode scalar values; publisher-date vs observed-text disagreement disclosed as `validity_conflict`, never resolved; renumbering detected mechanically (unique text-hash match) as `anchor_events`. Rule: every increment ends with something a stranger can experience. | amended 2026-08-15 |
...
| **D52** | EU expansion is configuration-led and temporal: engineering acquisition seeds plus bounded amendment, corrigendum, repeal, predecessor, successor, legal-basis and directly related delegated or implementing closure. Selection decides which works enter, never which official dated FR/EN expressions survive. Scope labels are provenance-bound engineering inputs, not legal metadata, filters, evidence or ranking fields. Loose citations do not recursively expand scope. | hybrid-eu/1, shipped |
...
| **D55** | Query indexes remain local. Container Apps serves a verified set within the active gates; any single artifact above 2 GiB blocks the current publication path and triggers a separately reviewed and authorized write-once-read-many publication design. A mounted set above 2 GiB triggers a zero-traffic VM benchmark, and a set above 4 GiB or a failed cold-start, latency, memory or two-release retention gate requires a VM-managed data disk. Private Blob storage is staging and coordination only; Blob and Azure Files are never the SQLite/vector query path or the current canonical release authority. | building; Container Apps path shipped, oversized releases blocked until publication and VM paths exist |
...
| **D64** | Search filter inventories come from verified mounted indexes, not a second hard-coded web list. Jurisdictions, hierarchies, act forms, binding statuses and languages therefore appear only when their indexed source data appears. The legacy generic `domain` field is empty in current v4 builds; official classifications are weak `matched_publisher_metadata` discovery values and may be selected only by repeating their exact official URI in `publisher_metadata_identifier`. Engineering acquisition-scope labels never enter corpus bytes or that inventory. | hybrid-eu/1, shipped |
...
| **D81** | Public assistant and MCP work is bounded before execution. The explicit public assistant default is 200 accepted turns per ingress-derived client and an independent 400 accepted turns globally per UTC day, with at most 4 executing concurrently. Assistant admission checks per-client quota, concurrency and global quota in that order; invalid, rejected, duplicate and oversized requests consume no accepted turn. Signed release evaluation uses separate bounded admission and consumes neither public daily counter; it never bypasses shared concurrency, queue, deadline, token or spend limits. Its short-lived reviewed envelope binds the exact candidate/catalog, each idempotency-key/request-body digest pair and the fresh-thread turn sequence; the runtime exchanges it for an opaque, bounded, single-process token and charges only an idempotency owner. MCP enforces a 64 KiB streamed-body limit, closed typed arguments, response collection and total legal-text budgets, 8 executing plus 16 queued calls, a 2 second queue deadline, 2 hybrid calls, and rolling 120/client and 600/global minute ceilings. Production remains one application replica while these process-local controls are authoritative. Raw prompts, opaque tokens and client addresses are excluded from error bodies and telemetry; only a closed metadata tag set is emitted. | implemented; production release proof pending |
