...
| **D55** | Hosting stays on Azure Container Apps Consumption with scale-to-zero. An always-on VM is reconsidered only after 30 measured days show at least 20% lower complete cost or Container Apps cannot satisfy the search latency gate. | hybrid-eu/1, shipped |
| **D56** | Legal hierarchy is normalized filter metadata, not a database boundary. Physical indexes follow publisher provenance and update semantics; one logical query spans them. Original publisher classifications remain available beside normalized fields. | hybrid-eu/1, building |
...
| **D64** | Search filter inventories come from verified mounted indexes, not a second hard-coded web list. Jurisdictions, hierarchies, reviewed domains, act forms, binding statuses and languages therefore appear when their indexed data appears. Stable lawyer-facing overrides label known vocabulary; unknown reviewed values receive a deterministic readable fallback. | hybrid-eu/1, building |
| **D65** | Offline semantic indexing embeds first-seen unique chunks in deterministic bounded batches. Batch size is an operational memory/throughput setting only: it cannot change chunk order, vector ordinals, content hashes or the single signed SQLite/vector artifact pair. Partial shards are not published or glued together. | hybrid-eu/1, building |
