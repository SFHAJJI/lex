...
| **D56** | Legal hierarchy is normalized filter metadata, not a database boundary. Physical indexes follow publisher provenance and update semantics; one logical query spans them. Original publisher classifications remain available beside normalized fields. | hybrid-eu/1, planned |
...
| **D58** | Artifact trust is anchored outside the artifact: a release-pinned public-key fingerprint verifies a signed canonical manifest covering databases, vectors, model, tokenizer, scope configuration, schema and source commits. The embedded stamp remains public provenance, not the runtime trust root. | hybrid-eu/1, planned |
| **D59** | Fuzzy search is a visible additive fallback only when exact lexical retrieval is weak. It never rewrites quotations, CELEX/ECLI identifiers, article numbers, dates or short tokens, and every expansion returns in the response. | hybrid-eu/1, planned |
