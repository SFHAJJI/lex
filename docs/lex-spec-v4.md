...
| **D79** | Conversation continuity is tab-scoped and server-stateless. The browser visibly retains at most six turns in `sessionStorage`; every request carries that bounded transcript to the server and Azure OpenAI when model planning or optional synthesis is needed. The server re-establishes only the most recent prior user-authored law identity through deterministic resolution, never from assistant prose, weak discovery or prior legal text. Prior context authorizes the matching typed operation but cannot suppress clarification for a new unresolved question. A visible new-conversation control clears the retained state while preserving the legal workspace; durable personal profiling is rejected. | shipped 2026-08-09; disclosure amended 2026-08-10 |
...
| **D81** | Public assistant and MCP work is bounded before execution. Assistant admission checks per-client quota, concurrency and global quota in that order; rejected, duplicate and oversized requests consume no model allowance. MCP enforces a 64 KiB streamed-body limit, closed typed arguments, response collection and total legal-text budgets, 8 executing plus 16 queued calls, a 2 second queue deadline, 2 hybrid calls, and rolling 120/client and 600/global minute ceilings. Production remains one application replica while these process-local controls are authoritative. Raw prompts and client addresses are excluded from error bodies and telemetry; only a closed metadata tag set is emitted. | building; implementation and burst tests complete, production proof pending |
