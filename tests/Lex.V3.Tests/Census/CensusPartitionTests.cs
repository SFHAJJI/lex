...
public sealed class CensusPartitionTests
{
    ...
    {
        Assert.AreEqual(
            602, ClosedSurfaceCensus.Candidates(CensusScope.SweptHere).Count, "candidates");
        Assert.AreEqual(
        ...
        Assert.AreEqual(
            74, ClosedSurfaceCensus.VocabularyRegistries(CensusScope.SweptHere).Count, "registries");
