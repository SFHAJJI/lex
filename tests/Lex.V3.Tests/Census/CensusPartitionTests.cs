...
public sealed class CensusPartitionTests
{
    ...
    {
        Assert.AreEqual(
            524, ClosedSurfaceCensus.Candidates(CensusScope.SweptHere).Count, "candidates");
        Assert.AreEqual(
            234, ClosedSurfaceCensus.ClosedVocabularies(CensusScope.SweptHere).Count, "vocabularies");
        Assert.AreEqual(
            176, ClosedSurfaceCensus.GuardedConstruction(CensusScope.SweptHere).Count, "guarded types");
        Assert.AreEqual(
