...
public sealed class LuxembourgTranspositionIdentityProducerTests
{
    ...
    public async Task TheProductionPathConsumesAReopenedLegiluxIdentityRow()
    {
        ...
        var request = new LuxembourgTranspositionIdentityRunRequest(plan, planResourceId, renderer);
        ...
        Assert.AreEqual(EuWorkKind.Directive, result.Relations[0].WorkKindAssertion.Kind);
        Assert.AreEqual(PublisherId.EuEurLex, result.Relations[0].WorkKindAssertion.Work.Publisher);
        Assert.AreEqual(EuEli, result.Relations[0].WorkKindAssertion.Work.Value(FactsIdentifierFamily.Eli));
    ...
    [TestMethod]
    public void MissingIdentityOrClassificationReachesTheTypedRefusal()
    {
