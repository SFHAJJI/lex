...
public sealed class CensusPartitionTests
{
    ...
    {
        Assert.AreEqual(
            134, ClosedSurfaceCensus.Candidates(CensusScope.SweptHere).Count, "candidates");
        Assert.AreEqual(
            54, ClosedSurfaceCensus.ClosedVocabularies(CensusScope.SweptHere).Count, "vocabularies");
        Assert.AreEqual(
            75, ClosedSurfaceCensus.GuardedConstruction(CensusScope.SweptHere).Count, "guarded types");
        Assert.AreEqual(
