...
export function renderTrustSurface() {
  const sections = [
    ...
    `<section class="surface-block"><h2>What leaves the building</h2>${renderEvidenceBundle({
      // No vocabulary is passed. Each item is labelled from its own publisher, which is the only
      // thing that works for a bundle, since a bundle may carry items from more than one.
      items: [
        {
          kind: 'publisher_text',
