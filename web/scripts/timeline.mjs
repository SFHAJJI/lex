...
function renderHoleRow(hole) {
  return (
    ...
    // This screen knows what this corpus holds. Saying no publisher state covers the period
    // claims the publisher's record is empty there, and the publisher may hold a state that
    // was never ingested. The derived note beside it already said absence of a held state
    // is not evidence, so the sentence above it was contradicting the sentence below.
