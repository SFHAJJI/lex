...
function hit(overrides = {}) {
  return {
    ...
    text_available: true,
    permalink: `https://law.soufien.lu/lu-legilux/code-travail/2001-01-01--${'a'.repeat(64)}`,
...
test('the zero-hit path requires the account too, which is where it matters most', () => {
  // The string renderer returns the no-hit card before it validates the account, so a zero-hit
  // screen can render with nothing declared. That is the one screen most likely to be read as
  // "the law does not say so", and the one where a silent rewrite matters most: the live service
  // answers this exact English query by expanding "many" into nonsense.
  assert.doesNotThrow(() =>
    renderSearchResults({
