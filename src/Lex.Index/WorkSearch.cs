...
public static class WorkSearch
{
    ...
    internal static IReadOnlyList<(long WorkId, string Reason, double Score)> Find(
        SqliteConnection connection, string query, string? language, int limit)
    {
        if (limit <= 0) return [];
        var normalized = Normalize(query);
        if (normalized.Length == 0) return [];
        var hits = new List<(long WorkId, string Reason, double Score)>();
        ...
        void AddFtsMatches(string ftsQuery, string reason)
        {
            ...
            while (matches.Read() && hits.Count < limit)
            {
                ...
                hits.Add((workId, reason, matches.GetDouble(1)));
            }
        }
        ...
        void AddExact(long workId, string kind, string prefix)
        {
            ...
            hits.Add((workId, $"{prefix}_{suffix}", -1000 + hits.Count));
        }
    }
