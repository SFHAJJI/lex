...
/// <summary>Resolves an existing path inside a trusted root without following links.</summary>
public static class ProtectedPath
{
    public static string RequireExisting(
        string root,
        string candidate,
        string description)
    {
        ...
        if (!string.Equals(fullCandidate, fullRoot, comparison)
            ...
            throw new InvalidDataException(
                $"{description} escapes its protected root: {candidate}");
    ...
    private static void RequireNotLink(string path, string description)
    {
        ...
        {
            throw new InvalidDataException(
                $"{description} cannot be verified: {path}", error);
        }
        if ((attributes & FileAttributes.ReparsePoint) != 0)
            throw new InvalidDataException(
                $"{description} contains a reparse point or symbolic link: {path}");
    }
}
